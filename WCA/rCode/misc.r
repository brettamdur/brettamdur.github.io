...
parseTractsBuckets <- function(df) {        
  ...
  ##################################################
 #### 1. NUMBER OF TRACTS PER INCOME BRACKET ######
##################################################
  ...
  ## Charts ##
    ...
    # histogram for values of median income by tract in all counties, faceted, with one histogram per county
      medianByTract %>% 
      ...
      labs(x = "Median Income (000s)", y = "Frequency (No. of Tracts)", title = "Number of Census Tracts by Median Income and County in NY State") +
      # labs(x = "Median Income (000s)", y = "Frequency (No. of Tracts)", title = "Distribution of Median Income by US Census Tract in Westchester County") +
      theme(
      ...
      nyIncomeDeviation <- medianByTract %>% 
              group_by(county) %>%
      ...
      nyIncomeDeviation %>% 
          ...
          coord_flip() +
          labs(x = "County", y = "Distance of Average Census Tract Income From of Median Income", title = "Distance of Average Census Tract Income from County Median by County in New York State") +
  ...
  #####################################################
 #### 2. NUMBER OF HOUSEHOLDS PER INCOME BRACKET #####
#####################################################
    ...
    ## Charts ##
      ...
      bucketsByTract %>% 
        ...
        labs(x = "Income", y = "Number of Households", title = "Number of Households by Income Bracket By County in New York State") +
        theme() +
...
#########################
    ...
    bucketsByTract_cleaned <- bucketsByTract %>% 
        ...
        #filter(county == "Westchester") %>%
        filter(county %in% comparableCounties) %>%
        # create a new column called startIncome, which is the the characters in "income" before the dash
    ...
    # lollipop chart ranking counties by distance from "perfect balance"
    waffleCounty %>% group_by(county) %>%
      ...
      # ggplot(aes(x = county, y = sd)) +#   geom_col() +
          ...
          geom_segment(aes(xend = county, yend = 0), color="blue") +
          coord_flip() +
...
# clear all variable from the environment
  rm(list = ls())