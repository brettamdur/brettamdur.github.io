...
function drawChart(medianByTract) {
    ...
    function drawIA(countyScope, yMax, chartSize){
        ...
        d3.csv("./data/bucketsByTract.csv").then(d => {
            ...
            // d3.select("#Westchester").remove()
        })
    }

    function drawLollipop() {

        // if there are any svg elements inside the class container2, stop execution of this function
        // if (d3.select(".container2").select("svg").size() > 0) {
        //     return;
        // }
    ...
    gsap.to("svg", {   
        scrollTrigger: {
            ...
            onEnter: function(){
                setTimeout(function(){
                    console.log("in step 4: " + step5Started);
                    if (step5Started) {
                        return;
                    }
                    drawLollipop();
            ...
            onLeaveBack: () => {
                // this is handled by step3:
                // d3.select(".lollipopDiv").remove();
            },
            onToggle: self => console.log(self.getVelocity())
        }
    })

    step5Animation = gsap.to("svg", {   
        scrollTrigger: {
            trigger: "#step5",  
            start: "top 20%",  
            end: "bottom 80%",
            ...
            fastScrollEnd: true,
            onEnter: function(){
                // ScrollTrigger.kill("trigger4")
                // gsap.killTweensOf(ScrollTrigger.getById("trigger4").animation)
                ...
                // . d3.selectAll(".lollipopDiv").remove(); 
                // select all the svgs inside container1 and remove them
                // d3.select(".container1").remove();
                console.log("step5started" + step5Started) 
            },
            onLeave: function(){
                // step5Started = false;
                console.log("step5started" + step5Started) 
