# FILE: wca_demographicsAnalysis.r
# AUTHOR: Brett Amdur
# DATE: 5/16/2023
# DESCRIPTION: 
    # r code for analysis of Westchester County, NY demographic data, 
    # used for project with the Westchester County Children's Association 
...
#####################
    ...
    setwd("/Users/jaycooney/Box/dataj/D3/WCA/rCode")
...
############################################################
  ...
  nydfCounty <- map_dfr(myTables, ~get_acs(
                      state = "NY",
                      geography = "county",
                      table = .x,
                      year = 2021
  ...
  # exporting nydf per Lim's request
  write.csv(nydf, "nydf.csv")
  ...
  bucketsByTractGrouped <- bucketsByTract %>% 
    # select counties to include
    filter(!county %in% c("Monroe", "Richmond")) %>%
    # filter(county == "Westchester") %>%
    filter(county %in% comparableCounties) %>%
...
# total household's check: matching data used in charts to total household data from ACS tables downloaded by county, without tract info
...
# data from ACS tables w/out tract info
parseTractsBuckets(parseTractsGeneral(nydfCounty)) %>%
  group_by(tract) %>%
  summarize(totalHouseholds = sum(estimate))